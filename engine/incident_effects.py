...
def resolve_murder(culprit: Character, incident: Incident, game_state: GameState):
    ...
    if len(others) >= 1:
        ...
        incident_choice = (
            todays_action.incident_choices[0]
            if todays_action.incident_choices
            else None
        )
        if incident_choice is None:
            raise ValueError(
                f"Murder incident should happen but no incident choice present"
            )
        ...
        print(
            f"💀 {culprit.name} (Murder Culprit) murdered {target_character.name} at {culprit.location.name}"
        )
    else:
        print(f"🟢 The murder incident does not happen on day {game_state.day}")
