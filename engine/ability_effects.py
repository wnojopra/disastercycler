...
def resolve_conspiracy_theorist_ability(
    source: Character, target: Character | Location
):
    assert isinstance(target, Character)
    # Find characters at the same location (excluding themself)
    if source.location != target.location:
        raise ValueError(
            f"Error resolving conspiracy theorist ability: target at different location"
        )
    else:
        target.paranoia += 1
